...
def hocbf_qp_control(state, u_nom, obstacle_center, obstacle_radius, safety_margin=0.5, gamma1=1.0, gamma2=1.0):
    ...
    input_lower_bound = ca.vertcat(-v_max, -delta_max)
    input_upper_bound = ca.vertcat(v_max, delta_max)
    ...
    omega_lower_bound = [0.0]
    omega_upper_bound = [0.0]
    ...
    try:
        sol = solver(lbg=ca.vertcat(input_lower_bound, cbf_lower_bound, omega_lower_bound), 
                        ubg=ca.vertcat(input_upper_bound, cbf_upper_bound, omega_upper_bound))
...
def compute_nominal_control(state, ref_point):
    ...
    # Feedback control
    v = kp_pos * d_pos 
    delta = np.clip(kp_delta * d_theta, -np.pi/4, np.pi/4)
...
plt.title("Non-Control Affine Circle Trajectory with 2nd Order CBF")
plt.legend()
plt.grid(True)
plt.show()
