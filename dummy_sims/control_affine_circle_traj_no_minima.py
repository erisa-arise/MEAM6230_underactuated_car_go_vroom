...
ref_lookahead = 1.0
obstacle_center = np.array([0.0, 10.0])
obstacle_radius = 1.0
safety_margin = 0.5
...
def cbf_qp_control(state, ref_point, u_nom, obstacle_center, obstacle_radius, safety_margin=0.5, gamma=1.0):
    x, y, theta = state
    ...
    dxh = grad_h[0] * ca.cos(theta) + grad_h[1] * ca.sin(theta)
    h_dot = v_sym * dxh
    ...
    obj = ca.sumsqr(u - u_nom) + rho.T @ rho

    nlp = {"x": u, "f": obj, "g": h_dot + gamma * h}
...
for _ in range(steps):
    ...
    u_safe = cbf_qp_control(state, ref_point, u_nom, obstacle_center, obstacle_radius, safety_margin)
